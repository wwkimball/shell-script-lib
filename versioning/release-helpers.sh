...
function getReleaseNumberForVersion {
	...
	if [ ! -d "$dataFileDir" ]; then
		if ! mkdir -p "$dataFileDir"; then
			echo "Unable to create data storage directory, ${dataFileDir}" >&2
			return 70
		fi
	fi
	...
	if [ -f "$dataFile" ]; then
		...
		if [ 0 -eq $? ]; then
			...
			then
				echo "Unable to save incremented release number to swap file, ${swapFile}." >&2
				return 71
			fi
			...
			if [ 1 -lt $rowCount ]; then
				if ! grep -v "^${recVersion}"$'\t' "$dataFile" >>"$swapFile"; then
					echo "Unable to copy other release records to swap file, ${swapFile}." >&2
					return 72
				fi
			fi
		else
			...
			then
				echo "Unable to add a new version record to swap file, ${swapFile}." >&2
				return 73
			fi

			if ! cat "$dataFile" >>"$swapFile"; then
				echo "Unable to transfer previous records from ${dataFile} to a swap file, ${swapFile}." >&2
				return 74
			fi
		fi

		if ! rm -f "$dataFile"; then
			echo "Unable to remove old data file, ${dataFile}." >&2
			return 75
		fi
		if ! mv "$swapFile" "$dataFile"; then
			echo "Unable to save updated data file, ${swapFile}, to ${dataFile}." >&2
			return 76
		fi
	else
		if ! echo -e "${versionNumber}\t${releaseNumber}\t$(date)\t$(date)" >"$dataFile"; then
			echo "Unable to create original data file, ${dataFile}." >&2
			return 77
		fi
	fi
